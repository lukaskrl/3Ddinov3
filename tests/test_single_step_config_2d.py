"""
Test a simple forward pass through the model using the ssl_default_config.yaml config for 2D images.
...
def test_model_forward_pass(tmp_path):
    """Test a simple forward pass through the model using ssl_default_config.yaml config for 2D images."""
    
    # 1. Load config from ssl_default_config.yaml
    config_path = Path(__file__).parent.parent / "dinov3" / "configs" / "ssl_default_config.yaml"
    ...
    # 4. Create dummy 2D input data matching the config
    # From config: global_crops_size: 224, local_crops_size: 96, batch_size_per_gpu: 64
    batch_size = cfg.train.batch_size_per_gpu
    n_global_crops = 2
    n_local_crops = cfg.crops.local_crops_number
    
    # Global crops: [n_global_crops, B, C, H, W]
    H = cfg.crops.global_crops_size
    W = cfg.crops.global_crops_size
    C = cfg.student.in_chans  # 3 for RGB images
    global_crops = torch.randn(n_global_crops, batch_size, C, H, W, device="cuda")
    
    # Local crops: [n_local_crops, B, C, H, W]
    H_local = cfg.crops.local_crops_size
    W_local = cfg.crops.local_crops_size
    local_crops = torch.randn(n_local_crops, batch_size, C, H_local, W_local, device="cuda")
    
    # 5. Forward pass through student backbone
    with torch.no_grad():
        # Flatten crops for backbone input
        global_crops_flat = global_crops.flatten(0, 1)  # [n_global_crops * B, C, H, W]
        local_crops_flat = local_crops.flatten(0, 1)    # [n_local_crops * B, C, H, W]
...
def test_loss_calculation(tmp_path):
    """Test loss calculation using SSLMetaArch with ssl_default_config.yaml config for 2D images."""
    
    # Set up single-process distributed environment (required for loss calculation)
    os.environ.setdefault("MASTER_ADDR", "127.0.0.1")
    os.environ.setdefault("MASTER_PORT", "29502")  # Different port from other tests
    ...
    try:
        ...
        # 6. Create dummy 2D input data matching the config
        batch_size = cfg.train.batch_size_per_gpu
        n_global_crops = 2
        n_local_crops = cfg.crops.local_crops_number
        ...
        # Format: [n_crops, B, C, H, W] -> flatten to [n_crops * B, C, H, W]
        global_crops = torch.randn(n_global_crops * batch_size, C, H, W, device="cuda")
        local_crops = torch.randn(n_local_crops * batch_size, C, H_local, W_local, device="cuda")
        
        # 7. Create masks for iBOT
        # Calculate number of tokens (2D: H_p * W_p)
        ...
        for i in range(n_global_crops * batch_size):
            if i < n_samples_masked:
                ...
                mask = torch.zeros((H_p, W_p), dtype=torch.bool)
